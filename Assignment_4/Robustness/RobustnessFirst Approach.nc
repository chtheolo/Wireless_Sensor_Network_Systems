...
module QueryPropagationC @safe()
{
	...
	uses interface Timer<TMilli> as TimerResponseUpdate;
...
{
/* --------------- POINTERS TO BUFFERS -------------- */
	response_update_msg_t *bcast_ReUpd, *rcv_ReUpd;
	update_msg_t *upd_bcast, *rcv_bcast_upd;
	query_flooding_msg_t *r_pkt, *bcast_pkt, *upd_ucast;
...
/*  8-bit  */
	...
	uint8_t sequence_number;					/* Counter used for recording the sequence numbers produced by an originator node.*/
	uint8_t data_id; // 8
	uint8_t s_data_id; // 8
	...
	task void FindQueryAndSend() {
		...
		while (query_pos < NUMBER_OF_QUERIES) {
			if (AQQ[query_pos].state == 1) {
				...
				bcast_ReUpd = (response_update_msg_t*) (call SamplingAMPacket.getPayload(&/*SamplingPacketBuffer[sampling_save]*/pkt, sizeof (response_update_msg_t)));
				if (bcast_ReUpd == NULL) {
					return;
				}
				//sampling_save++;

				bcast_ReUpd->source_id = AQQ[query_pos].source_id;
				bcast_ReUpd->sequence_number = AQQ[query_pos].sequence_number;
				bcast_ReUpd->forwarder_id = TOS_NODE_ID;
				bcast_ReUpd->father_node = AQQ[query_pos].father_node;
				bcast_ReUpd->hops = AQQ[query_pos].hops;
				bcast_ReUpd->sampling_period = AQQ[query_pos].sampling_period;
				bcast_ReUpd->query_lifetime = AQQ[query_pos].query_lifetime;
				bcast_ReUpd->propagation_mode = AQQ[query_pos].propagation_mode;
				bcast_ReUpd->remaining_period = TimeToMeasure[query_pos]; 
				mode = 4;

				if (AQQ[query_pos].propagation_mode == 1) {
					checkTimer = call Timer_StatsUnicast_Unicast.getNow();
					dt = checkTimer - time4MeasurementStartAt;
					bcast_ReUpd->remaining_period -= dt;
				}

				checkTimer = call TimerQueryFired.getNow(); /*calculate and send the remaing query lifetime*/
				dt = checkTimer - timerStartAt;
				bcast_ReUpd->query_lifetime -= dt;

				call Leds.led2On();
				call TimerResponseUpdate.startOneShot(20);
				break;
			}
			query_pos++;
		}
...
/* -------------------------------------------- TimerQueryBroadcast =>  SOURCE QUERY BROADCAST ------------------------------------------------- */ 	
	event void TimerQueryBroadcast.fired() {
		...
		if (!busy) {
			...
			if (mode == 4) {
				if (call RadioAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof (response_update_msg_t)) == SUCCESS){
					call Leds.led2Off();
					busy = TRUE;
					call Leds.led1On();
				}
			}	
			else if (mode == 3) {																				/*mode = 3 update bcast */
				if (call RadioAMSend.send(AM_BROADCAST_ADDR, &pkt, sizeof (update_msg_t)) == SUCCESS){
					busy = TRUE;
					call Leds.led1On();
				}
			}
			else if (mode == 2) {																			/*mode = 2, query cancellation bcast*/
			...
			else {																							/*mode = 0 OR mode = 1, query_flooding bcast*/
...
/* ------------------------------------ TimerResponseUpdate => UNICAST Response Update Info --------------------------------------- */
	event void TimerResponseUpdate.fired() {
		if(!unicast_busy) {
			if (call SamplingRadioAMSend.send(rcv_bcast_upd->node_id, &pkt, sizeof (response_update_msg_t)) == SUCCESS){
				unicast_busy = TRUE;
				call Leds.led2On();
			}
		}
...
/* ----------------------------------------- SAMPLING RADIO RECEIVE MESSAGES ------------------------------------------------ */
	event message_t* SamplingRadioReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof(sampling_msg_t)) {					/** RECEIVE SIMPLE SAMPLING MESSAGE **/
		...
		else if (len == sizeof(query_cancel_msg_t)) {				/*Query cancel message unicast*/
		...
		else if (len == sizeof(response_update_msg_t)) {			/*Response update unicast receive.*/
			rcv_ReUpd = (response_update_msg_t*) payload;

			query_pos = 0;									/*Search if i have received the same query before.*/
			while (query_pos < NUMBER_OF_QUERIES) {
				if (AQQ[query_pos].source_id == rcv_ReUpd->source_id && AQQ[query_pos].sequence_number == rcv_ReUpd->sequence_number) {
					if (rcv_ReUpd->hops < AQQ[query_pos].hops) {
						AQQ[query_pos].hops = rcv_ReUpd->hops;
						bcast_pkt->hops = rcv_ReUpd->hops;
					}
					break;
				}
				query_pos++;
			...
			if (query_pos == NUMBER_OF_QUERIES) {			/* It means that while-loop iterations finished and we didn't find the query. So save the new one.*/
				query_pos = 0;
				while (query_pos < NUMBER_OF_QUERIES) {
					if (AQQ[query_pos].state == 0) {
						AQQ[query_pos].source_id = rcv_ReUpd->source_id;
						AQQ[query_pos].sequence_number = rcv_ReUpd->sequence_number;
						AQQ[query_pos].forwarder_id = rcv_ReUpd->forwarder_id;
						AQQ[query_pos].father_node = rcv_ReUpd->father_node;
						AQQ[query_pos].hops = rcv_ReUpd->hops + 1;
						AQQ[query_pos].sampling_period = rcv_ReUpd->sampling_period;
						AQQ[query_pos].query_lifetime = rcv_ReUpd->query_lifetime;
						AQQ[query_pos].propagation_mode = rcv_ReUpd->propagation_mode;

						sendQuery = query_pos;
						post QueryScheduling();

						TimeToMeasure[sendQuery] = rcv_ReUpd->remaining_period;
						...
						call TimerQueryFired.startOneShot(rcv_ReUpd->remaining_period - 1000);		/*Upper bound before sending the broadcast acceptance.*/
						break;
					}
					query_pos++;
				}
...
/* ------------------------------------------- QUERY RADIO RECEIVE MESSAGES ------------------------------------------------- */
	event message_t* RadioReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof(query_flooding_msg_t)) {
		...
		else if (len == sizeof(query_cancel_msg_t)) {			/*QUERY CANCEL MESSAGE*/
			rcv_query_cacnel = (query_cancel_msg_t*) payload;
			...
			bcast_query_cancel->sequence_number = rcv_query_cacnel->sequence_number;
			bcast_query_cancel->propagation_mode = rcv_query_cacnel->propagation_mode;
			...
			//sendTofather = rcv_bcast_upd->node_id;
			if (number_Of_queries > 0) {
...
/* -------------------------------------------- QUERY SERIAL RECEIVE MESSAGE ------------------------------------------------------ */
	event message_t* SerialReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof (query_msg_t)) {
			...
			bcast_query_cancel->source_id = srl_query_cancel->source_id;
			bcast_query_cancel->sequence_number = srl_query_cancel->sequence_number;
			bcast_query_cancel->propagation_mode = srl_query_cancel->propagation_mode;
