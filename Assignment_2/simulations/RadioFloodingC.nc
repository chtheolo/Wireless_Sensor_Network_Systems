...
implementation
{
	...
	uint16_t latencyArr[LATSIZE];
	uint8_t latencyArrtime;//[LATSIZE];
	uint8_t ptype;
	unsigned char *str_time, *startTime;
...
//		uint8_t u, v=0, n=0;
//		for (u=0; u<3; u++) {
//			if (*(str_time+v) == ':' && *(str_time + v) == '.') {
//				v++;
//			}
//			else {
//				*(startTime+n) = *(str_time+v);
//				n++;
//			}
//			v++;
//		}
//		dbg("TimersC", "TIME = %s", startTime);
//	}
	...
	event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) {
		...
		if (len == sizeof (flooding_msg_t)) {
			...
			if (r_pkt->seq_num > seq_num) {
				...
				dbg("ReceiveC", "This is a NEW MESSAGE with seq_num=%hu @ %s.\n\n", r_pkt->seq_num, sim_time_string());
...
//				if (ptype < LATSIZE) {
//					latencyArr[ptype] = r_pkt->seq_num;
//					dbg("TimersC", "")sim_time_string();
//					ptype++;
//				}
