...
/* -------------------------------------------- Timer0 =>  SOURCE QUERY BROADCAST ------------------------------------------------- */ 	
	event void Timer0.fired() {
		dbg("BroadcastingC", "New Attempt for BROADCASTING QUERY_ID: %hu ,@ %s.\n", bcast_pkt->sequence_number, sim_time_string());
...
/* ---------------------------------------- Timer4 => Re-UNICAST MEASUREMENTS --------------------------------------------- */
	event void Timer4.fired() {
		if (!unicast_busy) {
			...
			if (ucast_pkt->source_id == TOS_NODE_ID && ucast_pkt->forwarder_id == TOS_NODE_ID) {
				dbg("RadioC", "UNICAST TRANSMISSION SUCCESS\n SOURCE_ID: %hu, FORWARDER_ID: %hu, DATA_ID: %hu \n\n", ucast_pkt->source_id, ucast_pkt->forwarder_id, ucast_pkt->data_id);
			}
			else {
				dbg("RadioC", "RE-UNICAST TRANSMISSION SUCCESS\n SOURCE_ID: %hu, FORWARDER_ID: %hu, DATA_ID: %hu \n\n", ucast_pkt->source_id, ucast_pkt->forwarder_id, ucast_pkt->data_id);
			}
...
/* ----------------------------------------- SAMPLING RADIO RECEIVE MESSAGES ------------------------------------------------ */
	event message_t* SamplingRadioReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof(sampling_msg_t)) {
			...
			dbg("ReceiveC", "NEW Samplings Received \n");
			dbg("ReceiveC", "SOURCE_ID: %hu, FORWARDER_ID: %hu, SAMPLING_ID: %hu \n", r_sampling_pkt->source_id, r_sampling_pkt->forwarder_id, r_sampling_pkt->data_id);
