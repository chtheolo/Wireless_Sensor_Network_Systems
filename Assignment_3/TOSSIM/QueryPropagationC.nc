...
/*  8-bit  */
	...
	uint8_t next;
	uint8_t count_contributed_nodes;
...
/* ----------------------------------------------- Measurement Scheduling -------------------------------------------------- */
	task void MeasurementScheduling() {
		if (call Timer1.isRunning() == TRUE) {
			...
			/** The query that just came is the only one so it is free to take the Timer1 for sampling period */
			Hold_Sampling_Timer = sendQuery; 
			dbg("ReceiveC", "TASK Sampling Period: %hu \n", TimeToMeasure[Hold_Sampling_Timer]);
			call Timer1.startOneShot(TimeToMeasure[Hold_Sampling_Timer]);
			time4MeasurementStartAt = call Timer1.getNow();
		}
	}
	...
	event void Read.readDone(error_t result, uint16_t data) {
		if(result == SUCCESS){
			...
			if (TOS_NODE_ID == AQQ[Hold_Sampling_Timer].source_id) { // if i am the query originator
				...
				ucast_pkt->count_contributed_nodes = count_contributed_nodes++;

				sendTofather = AQQ[Hold_Sampling_Timer].forwarder_id;	

				call Timer5.startOneShot(TOS_NODE_ID * 20); // 50
			}
...
/* ------------------------------------------- Timer3 => Query_Lifetime END ------------------------------------------------- */ 
	event void Timer3.fired() {
		dbg("QueryC", "QUERY_ID: %hu => EXPIRED!!! @ %s \n", AQQ[HoldTimer].sequence_number, sim_time_string());
	...
	event message_t* SamplingRadioReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof(sampling_msg_t)) {
			...
			if (r_sampling_pkt->destination_id != TOS_NODE_ID) {
				...
				ucast_pkt->count_contributed_nodes = r_sampling_pkt->count_contributed_nodes + 1;
				...
				call Timer4.startOneShot(TOS_NODE_ID * 15); // Re-Unicast the received sampling packet
			}
				...
				count_contributed_nodes = r_sampling_pkt->count_contributed_nodes + 1;

				call Timer2.startOneShot(20);  // serial mote to pc
			}
		}
		return msg;
	}
		...
		if (len == sizeof(query_flooding_msg_t)) {
			...
			if (query_pos < LAST_SENDERS && r_pkt->sequence_number > QuerySendersHistory[query_pos].sequence_number) {
				...
				if (number_Of_queries < NUMBER_OF_QUERIES) {
					...
					dbg("ReceiveC", "NEW Radio Received QUERY_ID: %hu, SOURCE_ID: %hu, FORWARDER_ID: %hu ,@ %s \n\n", r_pkt->sequence_number, r_pkt->source_id, r_pkt->forwarder_id, sim_time_string());
	...
	event message_t* SerialReceive.receive(message_t* msg, void* payload, uint8_t len) {
		if (len == sizeof (query_msg_t)) {
			...
			if (number_Of_queries < NUMBER_OF_QUERIES) {
				...
				dbg("ReceiveC", "The Sampling Period for query_id: %hu is T: %hu \n", AQQ[sendQuery].sequence_number, AQQ[sendQuery].sampling_period);
