...
/* ------------------------------- Query Canceling -------------------------------- */
	task void QueryCancel() {
		if (HoldTimer == query_cancel) {							/*if query_cancel is the one that holds the timer, then*/
			checkTimer = call TimerQueryFired.getNow();
			runningTime = checkTimer - timerStartAt;				/*find the active time, and...*/
			AQQ[query_cancel].query_lifetime = runningTime;			/* ...save it. Stop timer and call again in order to re-scheduling the other queries*/
			call TimerQueryFired.stop();
			call TimerQueryFired.startOneShot(10);
		}
		else {
			AQQ[query_cancel].state = 0;							/*if query_cancel != hold timer, then just update the query state to 0.*/
		}

		if (call TimerQueryBroadcast.isRunning() == TRUE) {			/*Re-Broadcast the query cancellation to the rest network.*/
			t0 = call TimerQueryBroadcast.gett0();
			dt = call TimerQueryBroadcast.getdt();
			call TimerQueryBroadcast.startOneShot(t0 + dt);
		}
...
/* ------------------------- TimerQueryCancelResponse =>  WAITING UPPER BOUND TIME, TO CHECK WHO SEND CANCEL CONFIRMATION -------------------------- */ 
